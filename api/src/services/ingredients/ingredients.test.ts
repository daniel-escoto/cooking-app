...
describe('ingredients', () => {
  ...
  scenario('creates a ingredient', async (scenario: StandardScenario) => {
    const result = await createIngredient({
      input: {
        name: 'String',
        quantity: 'String',
        measure: 'String',
        recipeId: scenario.ingredient.two.recipeId,
      },
    })

    expect(result.name).toEqual('String')
    expect(result.quantity).toEqual('String')
    expect(result.measure).toEqual('String')
    expect(result.recipeId).toEqual(scenario.ingredient.two.recipeId)
  })
